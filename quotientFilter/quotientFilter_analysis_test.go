...
func TestSizeInFunctionOfErrorRate(t *testing.T) {
	...
	for k, pr := range proofs {
		f := NewFromSizeAndError(uint(n), pr.e)
		println(f.m, f.r, f.TotalSize())
		results[k] = []string{fmt.Sprint(pr.e), fmt.Sprint(f.TotalSize())}
	}
	resultsFile, err := os.Create(fmt.Sprintf("../results/QF_size_n:%d.csv", n))
	if err != nil {
		t.Fatal(err)
	}
	w := csv.NewWriter(resultsFile)
...
func TestBitsPerSlotInFunctionOfErrorRate(t *testing.T) {
	...
	for k, pr := range proofs {
		f := NewFromSizeAndError(uint(n), pr.e)
		results[k] = []string{fmt.Sprint(pr.e), fmt.Sprint(f.r+3)}
	}
	resultsFile, err := os.Create(fmt.Sprintf("../results/QF_bits_per_element_n:%d.csv", n))
	if err != nil {
		t.Fatal(err)
	}
	w := csv.NewWriter(resultsFile)
